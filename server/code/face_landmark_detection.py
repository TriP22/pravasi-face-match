...
def crop_image(img1, img2):
    [size1, size2, diff0, diff1, avg0,
        avg1] = calculate_margin_help(img1, img2)
    ...
    if (size1[0] == size2[0] and size1[1] == size2[1]):
        ...
        if (scale0 > scale1):
            res = cv2.resize(img2, None, fx=scale0, fy=scale0,
                             interpolation=cv2.INTER_AREA)
        else:
            res = cv2.resize(img2, None, fx=scale1, fy=scale1,
                             interpolation=cv2.INTER_AREA)
        ...
        if (scale0 > scale1):
            res = cv2.resize(img1, None, fx=scale0, fy=scale0,
                             interpolation=cv2.INTER_AREA)
        else:
            res = cv2.resize(img1, None, fx=scale1, fy=scale1,
                             interpolation=cv2.INTER_AREA)
...
def generate_face_correspondences(theImage1, theImage2):
    ...
    for img in imgList:
        ...
        try:
            ...
            print("Sorry, but I couldn't find a face in the image.", j)
